...
create table [hris].[employee_position_fact] (
    ...
    [started_position_date_key] int  not null
);
go

-- creating primary key on [employee_position_key] in table 'employee_position_fact'
alter table [hris].[employee_position_fact]
...
on [hris].[employee_position_fact]
    ([position_key]);
go
...
-- creating foreign key on [started_position_date_key] in table 'employee_position_fact'
alter table [hris].[employee_position_fact]
add constraint [fk_date_dimemployee_position_fact]
    foreign key ([started_position_date_key])
    references [hris].[date_dim]
        ([date_key])
    on delete no action on update no action;
go

-- creating non-clustered index for foreign key 'fk_date_dimemployee_position_fact'
create index [ix_fk_date_dimemployee_position_fact]
on [hris].[employee_position_fact]
    ([started_position_date_key]);
go
